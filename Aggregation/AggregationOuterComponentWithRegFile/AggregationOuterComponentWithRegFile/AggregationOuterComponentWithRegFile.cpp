...
HRESULT __stdcall CSumSubtract::SumOfTwoIntegers(int m, int n, int* r) {
	*r = m + n;
	return(S_OK);
}

HRESULT __stdcall CSumSubtract::SubtractionOfTwoIntegers(int m, int n, int* r) {
	*r = m - n;
	return(S_OK);
}
...
HRESULT __stdcall CSumSubtract::InitializeInnerComponent(void) {
	...
	hr = CoCreateInstance(CLSID_IMultiplicationDivision, reinterpret_cast<IUnknown*>(this), CLSCTX_INPROC_SERVER, IID_IUnknown, (void**)&m_pIUnknownInner);
	if (FAILED(hr)) {
